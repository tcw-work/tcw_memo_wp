...
$(function () {
  ...
  function makeMokuji() {
    ...
    $('.article_content h2, .article_content h3, .article_content h4, .article_content h5, .article_content h6').each(function (i) {
      // 見出しごとにIDを保存
      this.id = 'autoid_' + idcount; //			this.class = 's_h_' + idcount;
    ...
    var elements = $('[id^="autoid_"]'); // 要素の選択

    var heading_tops = $.map(elements, function (element) {
      // ループして offset().top を取得して新しい配列に格納
    ...
    $(window).on("scroll", function () {
      ...
      for (var i = 0; i < heading_tops.length; i++) {
        // 「if」文の条件分岐をループ処理に変更
      ...
      $(".autoid_".concat(activeIndex + 1, " a")).addClass('s_heading_active'); // アクティブな見出しのインデックスを変数「activeIndex」で管理するように変更しました。
    });
  }
